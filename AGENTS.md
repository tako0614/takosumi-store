...
- Scope: Decentralized catalog and install-link surface for discoverable Capsules.
- Repository kind: `product`
- Direct sibling dependencies: なし
- Repository gate: `bun run check`
- Canonical docs: [README.md](README.md), [docs/SPEC.md](docs/SPEC.md)

## Ownership

- Owns: TCS open read API contract and implementation / Catalog listing, publishing, moderation, and UI / Server-local presentation and curation metadata
- Does not own: Capsule install, apply, or source-version selection / Install manifests, setup, or output projection / Cross-server publisher trust
- Hazards: The store is not an installable Capsule until it ships an OpenTofu module. / Retired envelope and release-candidate flows are not current authority.
