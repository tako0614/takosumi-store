...
function installLiveFetchMock(): void {
  globalThis.fetch = (async (input, init) => {
    ...
    if (url.pathname === "/.well-known/tcs") {
      return Response.json({
        server: {
          software: { name: "takosumi-store", version: "0.1.7" },
...
describe("release operation journal retry", () => {
  test("resumes monotonically from every retained phase", async () => {
    ...
    for (const phase of [
      ...
      const envelope = {
        releaseId: "takosumi-store-0.1.7-retry",
      ...
      const manifest = {
        ...
        assets: [
          { path: "assets/index-review.js", sha256: sha256Bytes(staticBytes) },
          { path: "index.html", sha256: sha256Bytes(indexBytes) },
        ],
